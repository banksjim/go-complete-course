...
func calculateFutureValues(investmentAmount float64, expectedReturnRate float64, yearsInvested float64) (fv float64, rfv float64) {
	...
	// Note the use of specifying the return values in the function definition
	// but ALSO using explicit returns. This approach is good for documentation, but not the most preferred way.
	return fv, rfv
}
